 ...
 group_by(repN,  prior.info) %>%
  ...
  group_by(N_event_look, prior.info) %>%
    filter(N_event_look==eval.points[length(eval.points)] ) %>%
       summarize(prop_stopped = mean(stop), ave_pop_stop = mean(pop*stop_point, na.rm=T)) %>%
...
threshold_application <- function( set.eval.points){
   ...
   min.tresholds <- seq.tresholds %>%
    ...
    filter(rank_acceptable==1) %>%
     select(prior.info,test.threshold,ave_pop_stop)
...
ggplot(aes( x=pop)) +
  ...
  #ggtitle('Cumulative proportion with >X% prob that effect is >0%VE')
   ...
   out.obj = list('min.tresholds'=min.tresholds, 'stopped_efficacy1'=stopped_efficacy1,'seq.tresholds'=seq.tresholds,'p1'=p1,'p2'=p2,'p3'=p3)
   return(out.obj)
}
```
