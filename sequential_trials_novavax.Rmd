...
### Frequentist power for fixed sample size
...
### Power for sequential trial with looks every 600 people, no control of type I error

Figure S2

```{r, fig.width=6, fig.height=3}
stopped_efficacy1 =probs %>%
  filter(pop %in% c(600, 1200,1800, 2400, 3000) & prior.info %in% c("N(0,3.32)", "N(u1,tau2); tau2~IG(0.01,0.01)", "N(u1,tau); tau~U(0,2)"  )) %>%
  arrange(repN, ve.new.trial,prior.info ,pop) %>%
 group_by(repN,  ve.new.trial,prior.info) %>%
...
Every 600, starting at 600 vaccinees (1200 total subjects)
```{r}

seq.results1 <-threshold_application( set.eval.point=c(600, 1200,1800, 2400, 3000))

seq.results1$p1

seq.results1$p2

ggplotly(seq.results1$p2)
tab1 <- seq.results1$min.tresholds %>%
  mutate(startpop=600, endpop=3000, freq_look=600)
