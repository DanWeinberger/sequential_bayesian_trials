...
threshold_application <- function( set.eval.points){
   ...
   stopped_efficacy1 =probs %>%
    filter(pop %in% set.eval.points & prior.info %in% c("N(0,1.821^2)", "N(u1,tau); IG(0.01,0.01)", "N(u1,tau); U(0,2)" )) %>%
   ...
   out.obj = list('min.tresholds'=min.tresholds, 'stopped_efficacy1'=stopped_efficacy1,'seq.tresholds'=seq.tresholds)
   return(out.obj)
}
```
...
seq.results <-threshold_application( set.eval.point=seq(500,2000, by=500))

##Plot the outputs
p1 <- ggplot(seq.results$seq.tresholds, aes( x=test.threshold, y=prop_stopped, group=prior.info, color=prior.info)) + 
  geom_line() +
  theme_classic() +
  geom_hline(yintercept=0.05)
p1

p2 <- seq.results$stopped_efficacy1 %>%
ggplot(aes( x=pop, y=prop_stopped, group=prior.info, color=prior.info)) +
  geom_line() +
    geom_point() +
  facet_wrap(~ ve.new.trial) +
