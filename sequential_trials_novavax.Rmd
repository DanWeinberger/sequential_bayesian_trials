...
stopped_efficacy1 =probs %>%
  filter(N_event_look %in% c(10,20,30,40,50,100,150) & prior.info %in% c("N(0,3.32)", "N(u1,tau2); tau2~IG(0.01,0.01)", "N(u1,tau); tau~U(0,2)"  )) %>%
  arrange(repN, ve.new.trial,prior.info ,N_event_look) %>%
 group_by(repN,  ve.new.trial,prior.info) %>%
...
seq.results1 <-threshold_application( set.eval.point=c(50,100,150))
...
seq.results2 <-threshold_application( set.eval.point=seq(50,140, by=20))
...
tab2 <- seq.results2$min.tresholds %>%
  mutate(startpop=50, endpop=140, freq_look=20) 
...
seq.results3 <-threshold_application( set.eval.point=seq(50,150, by=10))
...
seq.results4 <-threshold_application( set.eval.point=seq(70,150, by=10))
...
```{r}
tab1 <- bind_rows(tab1, tab2,tab3, tab4) %>% 
  filter(prior.info %in% c("N(0,3.32)", "N(u1,tau2); tau2~IG(0.01,0.01)", "N(u1,tau); tau~U(0,2)"  )) %>%
