...
ggplot( aes(x=pop, y=p_any_benefit0, group=interaction(repN,prior.info), color=Pooling_type)) +
  geom_line( alpha=0.1) +
  theme_classic() +
...
What proportion of trials would have a P(benefit\>0.975\|data) at different sample sizes? 


for each trial, call 0/1 based on if we have hit 0.975 at this sample size or earlier

```{r, fig.width=4, fig.height=2}
power1 =probs %>%
  arrange(repN, ve.new.trial,prior.info ,pop) %>%
 group_by(repN,  ve.new.trial,prior.info) %>%
  mutate( eff_any = if_else(p_any_benefit0>0.975,1,0),
          eff_10 = if_else(p_moderate_0_90>0.975,1,0),
          eff_25 = if_else(p_moderate_0_75>0.975,1,0),
          futile_30 = if_else(p_futile>0.975,1,0),
            ) %>%
  ungroup() %>%
  group_by(pop, ve.new.trial,prior.info) %>%
    summarize(prop_eff_any = mean(eff_any),
...
power1 %>%
ggplot(aes( x=pop, y=prop_eff_any, group=prior.info, color=prior.info)) +
  geom_line() +
  facet_wrap(~ ve.new.trial) +
  theme_classic() +
  geom_hline(yintercept = c(0.05, 0.8), lty=2, col='gray')+
  ggtitle('Proportion of trials with >97.5% probability that effect is >0% VE')
```
P VE >10%
```{r, fig.width=4, fig.height=2}
power1 %>%
ggplot(aes( x=pop, y=prop_eff_10, group=prior.info, color=prior.info)) +
  ...
  theme_classic() +
  geom_hline(yintercept = c(0.05, 0.8), lty=2, col='gray')+
  ggtitle('Proportion of trials with >97.5% probability that effect is >10% VE')
```

P VE >25%
```{r, fig.width=4, fig.height=2}
power1 %>%
ggplot(aes( x=pop, y=prop_eff_25, group=prior.info, color=prior.info)) +
  geom_line() +
  facet_wrap(~ ve.new.trial) +
  theme_classic() +
  geom_hline(yintercept = c(0.05, 0.8), lty=2, col='gray')+
  ggtitle('Proportion of trials with >97.5% probability that effect is >25% VE')
```
...
ggplot(aes( x=pop, y=prop_futile_30, group=prior.info, color=prior.info)) +
  ...
  theme_classic() +
  geom_hline(yintercept = c(0.05, 0.8), lty=2, col='gray')+
  ggtitle('Proportion of trials with >97.5% probability that effect is <30% VE')
```
```{r}
ggplot(power1, aes( x=prop_futile_30, y=prop_eff_any, group=prior.info, color=prior.info)) +
  geom_point() +
  theme_classic() +
  facet_wrap(~ ve.new.trial)
```
...
### Bayesian power for sequential trial with looks every 100 people

Evaluate cumulate probability of hittig the threshold
```{r, fig.width=6, fig.height=3}
stopped_efficacy1 =probs %>%
  arrange(repN, ve.new.trial,prior.info ,pop) %>%
 group_by(repN,  ve.new.trial,prior.info) %>%
  mutate( eff = 1*(p_any_benefit0>0.975),
            stop = cumsum(eff) ,
...
stopped_efficacy1 =probs %>%
  arrange(repN, ve.new.trial,prior.info ,pop) %>%
 group_by(repN,  ve.new.trial,prior.info) %>%
  mutate( eff = 1*(p_moderate_0_90>0.975),
            stop = cumsum(eff) ,
          stop = if_else(stop>=1,1,0)) %>%
  ungroup() %>%
  group_by(pop, ve.new.trial,prior.info) %>%
...
stopped_efficacy2 =probs %>%
  arrange(repN, ve.new.trial,prior.info ,pop) %>%
 group_by(repN,  ve.new.trial,prior.info) %>%
  mutate( eff = 1*(p_moderate_0_75>0.975),
            stop = cumsum(eff) ,
          stop = if_else(stop>=1,1,0)) %>%
  ungroup() %>%
  group_by(pop, ve.new.trial,prior.info) %>%
...
stopped_efficacy3 =probs %>%
  arrange(repN, ve.new.trial,prior.info ,pop) %>%
 group_by(repN,  ve.new.trial,prior.info) %>%
  mutate( eff = 1*(p_any_benefit0>0.999),
            stop = cumsum(eff) ,
          stop = if_else(stop>=1,1,0)) %>%
  ungroup() %>%
  group_by(pop, ve.new.trial,prior.info) %>%
...
stopped_efficacy3 =probs %>%
  arrange(repN, ve.new.trial,prior.info ,pop) %>%
 group_by(repN,  ve.new.trial,prior.info) %>%
  mutate( eff = 1*(p_any_benefit0>0.99),
            stop = cumsum(eff) ,
          stop = if_else(stop>=1,1,0)) %>%
  ungroup() %>%
  group_by(pop, ve.new.trial,prior.info) %>%
...
stopped_futility =probs %>%
 ...
 group_by(repN, pop, ve.new.trial,prior.info) %>%
  mutate( fut = 1*((1-p_any_benefit0)>0.5),
            stop = cumsum(fut) ,
          stop = if_else(stop>=1,1,0)) %>%
  ungroup() %>%
  group_by(pop, ve.new.trial,prior.info) %>%
...
ggplot( aes(x=pop, y=p_any_benefit0_mean,  color=interaction(Pooling_type,prior.info), group=interaction(Pooling_type,prior.info), fill=Pooling_type)) +
  geom_line() +
...
p1 <-probs %>%
  ...
  mutate(Pooling_type = as.factor(Pooling_type), ve.new.trial=as.factor(ve.new.trial)) %>%
  group_by(pop, ve.new.trial,Pooling_type,prior.info) %>%
  summarize( proportion_any_benefit= mean(p_any_benefit0>0.975 )) %>%
...
probs %>%
  ...
  mutate(Pooling_type = as.factor(Pooling_type), ve.new.trial=as.factor(ve.new.trial)) %>%
  group_by(pop, ve.new.trial,Pooling_type,prior.info) %>%
  summarize( p_nontrivial_benefit= mean(p_nontrivial_0_95>0.975)) %>%
...
probs %>%
  ...
  mutate(Pooling_type = as.factor(Pooling_type), ve.new.trial=as.factor(ve.new.trial)) %>%
  ggplot(aes(x=p_similar_0_8__1_25, y=p_any_benefit0)) +
  geom_point() +
  theme_classic()
```
...
prop_pop_90 <- all.mods %>%
  ...
  summarize( prop_any_effect=mean(p_any_benefit0 >=0.9)*100 )
