...
stopped_efficacy1 =probs %>%
  filter(total_pop %in% c(1200, 2400,3600, 4800, 6000) & prior.info %in% c("N(0,3.32)", "N(\u03B4\u2081,\u03C3\u00B2); \u03C3\u00B2~IG(0.01,0.01)", "N(\u03B4\u2081,\u03C3\u00B2); \u03C3~U(0,2)"  )) %>%
  arrange(repN, ve.new.trial,prior.info ,total_pop) %>%
 group_by(repN,  ve.new.trial,prior.info) %>%
...
stopped_futility =probs %>%
    filter(total_pop %in% c(1200, 2400,3600, 4800, 6000)) %>%
...
Every 1200, starting at 1200 Participants
```{r}
...
```{r, fig.width=6, fig.height=3}
stopped_efficacy1 =probs %>%
  filter(pop %in% seq(500, 2000, by=100) & prior.info %in% c("N(0,3.32)", "N(\u03B4\u2081,\u03C3\u00B2); \u03C3\u00B2~IG(0.01,0.01)", "N(\u03B4\u2081,\u03C3\u00B2); \u03C3~U(0,2)"  )) %>%
  arrange(repN, ve.new.trial,prior.info ,pop) %>%
 group_by(repN,  ve.new.trial,prior.info) %>%
