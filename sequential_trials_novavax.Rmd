...
Prior for VE proposed https://hbiostat.org/proj/covid19/bayesplan.html#operating-characteristics (low probability of extreme effects, symmetric around 0)
A skeptical prior as defined by Frank harrell has low probability of extremes (e.g., 5% prior probability that the effect is >95% or 5% that it is the inverse )
```{r}
...
all.mods <- bind_rows(mod0,mod1, mod3, mod4, mod5, mod6)
...
probs %>%
  filter(ve.new.trial==44.4 & Pooling_type != 'Full') %>%
  mutate(Pooling_type = as.factor(Pooling_type)) %>%
ggplot( aes(x=pop, y=p_moderate_0_75, group=interaction(repN,prior.info), color=Pooling_type)) +
  geom_line( alpha=0.1) +
  theme_classic() +
  facet_wrap(~Pooling_type)
```
...
ggplot( aes(x=pop, y=p_any_benefit0_mean,  color=interaction(Pooling_type,prior.info), group=interaction(Pooling_type,prior.info), fill=Pooling_type)) +
  ...
  theme_classic() +
  facet_wrap(Pooling_type~ve.new.trial)+
