...
power1 =probs %>%
 ...
 group_by(repN,  ve.new.trial,prior.info) %>%
  mutate( eff_any = if_else(p_any_benefit0>0.95,1,0),
          eff_10 = if_else(p_moderate_0_90>0.95,1,0),
          eff_25 = if_else(p_moderate_0_75>0.95,1,0),
...
ggplot(aes( x=pop, y=prop_eff_25, group=prior.info, color=prior.info)) +
  ...
  theme_classic() +
  geom_hline(yintercept = c(0.05, 0.8), lty=2, col='gray')+
  ggtitle('Proportion of trials with >95% probability that effect is >25% VE')
```
...
Evaluate cumulative probability of hitting the threshold
...
stopped_efficacy1 =probs %>%
  ...
  arrange(repN, ve.new.trial,prior.info ,pop) %>%
 group_by(repN,  ve.new.trial,prior.info) %>%
  mutate( eff = 1*(p_any_benefit0>0.95),
            stop = cumsum(eff) ,
...
ggplot(aes( x=pop, y=prop_stopped, group=prior.info, color=prior.info)) +
 ...
 stopped_efficacy1 =probs %>%
  ...
  arrange(repN, ve.new.trial,prior.info ,pop) %>%
 group_by(repN,  ve.new.trial,prior.info) %>%
  mutate( eff = 1*(p_moderate_0_90>0.95),
            stop = cumsum(eff) ,
          stop = if_else(stop>=1,1,0)) %>%
  ungroup() %>%
  group_by(pop, ve.new.trial,prior.info) %>%
...
stopped_efficacy2 =probs %>%
  ...
  arrange(repN, ve.new.trial,prior.info ,pop) %>%
 group_by(repN,  ve.new.trial,prior.info) %>%
  mutate( eff = 1*(p_moderate_0_75>0.95),
            stop = cumsum(eff) ,
          stop = if_else(stop>=1,1,0)) %>%
  ungroup() %>%
  group_by(pop, ve.new.trial,prior.info) %>%
...
ggplot(aes( x=pop, y=prop_stopped, group=prior.info, color=prior.info)) +
  ...
  theme_classic() +
  geom_hline(yintercept = c(0.05, 0.8), lty=2, col='gray')+
  ggtitle('Cumulative proportion with >95% prob that effect is >25%VE')
...
p1 <-probs %>%
  ...
  mutate(Pooling_type = as.factor(Pooling_type), ve.new.trial=as.factor(ve.new.trial)) %>%
  group_by(pop, ve.new.trial,Pooling_type,prior.info) %>%
  summarize( proportion_any_benefit= mean(p_any_benefit0>0.95 )) %>%
ggplot( aes(x=pop, y=proportion_any_benefit,  color=interaction(Pooling_type,prior.info), group=interaction(Pooling_type,prior.info))) +
  ...
  ggtitle('Proportion of trials showing probability of benefit >95%')
...
probs %>%
  ...
  mutate(Pooling_type = as.factor(Pooling_type), ve.new.trial=as.factor(ve.new.trial)) %>%
  group_by(pop, ve.new.trial,Pooling_type,prior.info) %>%
  summarize( p_nontrivial_benefit= mean(p_nontrivial_0_95>0.95)) %>%
ggplot( aes(x=pop, y=p_nontrivial_benefit,  color=interaction(Pooling_type,prior.info), group=interaction(Pooling_type,prior.info))) +
  ...
  facet_wrap(~ve.new.trial)+
  geom_hline(yintercept=c(0.05,0.8), lty=2, col='gray') +
  ggtitle('Proportion of trials showing >95% probability of non-trivial benefit (VE>5%)')
```
