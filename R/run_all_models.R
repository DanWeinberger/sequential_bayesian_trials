...
## Informative prior, but with Alpha to downweight variance if needed
  ...
  # mod5a <- lapply(1:N.sim, function(x) call_jags(sim.ds=sim.ds1,prior.mean=prior.data$log_irr.obs[1], prior.prec=prior.data$prec.log.irr.obs,repN=x, model.select=model_string_modified_pois_mixture) )
  # 
  # mod5b <- lapply(1:N.sim, function(x) call_jags(sim.ds=sim.ds2,prior.mean=prior.data$log_irr.obs[1], prior.prec=prior.data$prec.log.irr.obs,repN=x, model.select=model_string_modified_pois_mixture) )
  # 
